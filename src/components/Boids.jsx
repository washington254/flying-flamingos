...
export const Boids = ({ boundaries }) => {
  ...
  const { NB_BOIDS, MIN_SCALE, MAX_SCALE, MIN_SPEED, MAX_SPEED, MAX_STEERING } =
    useControls(
      "General settings",
      {
        NB_BOIDS: { value: 150, min: 30, max: 200 },
        MIN_SCALE: { value: 0.4, min: 0.1, max: 2, step: 0.1 },
        MAX_SCALE: { value: 1.0, min: 0.1, max: 2, step: 0.1 },
        MIN_SPEED: { value: 0.5, min: 0, max: 10, step: 0.1 },
        MAX_SPEED: { value: 1.5, min: 0, max: 10, step: 0.1 },
        MAX_STEERING: { value: 0.02, min: 0, max: 1, step: 0.01 },
      },
      { collapsed: true }
    );
